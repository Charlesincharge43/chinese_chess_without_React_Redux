...
{
  console.log('Inside drawTeamofPieces, state for ',teamOfPieces);
...
function getPieceAtBlockForTeam(teamOfPieces, clickedLocObj)
{
    ...
    for (let pieceKey in teamOfPieces)
    {
        curPiece=teamOfPieces[pieceKey];
...
function selectPiece(pieceAtBlock)//note.. the selectedKey happens at getPieceAtBlockForTeam, not in this function
{
...
function processMove(clickedLocObj)
{
  ...
  for(let hitBoxSqObj of moveHitBoxesArr){
    ...
    {
      targetX=hitBoxSqObj.xCent;
      targetY=hitBoxSqObj.yCent;

      let test='red'
      let pieceObjToMove=json[test][selectedKey];
      pieceObjToMove.x=hitBoxSqObj.xCent;
      pieceObjToMove.y=hitBoxSqObj.yCent;

      console.log('before drawpieces ',json.red)
      ctx.strokeStyle = BLACK;
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      drawBoard();
      drawPieces();
      // console.log('after drawpieces ',json.red)
...
function reDraw(){
  ...
  drawBoard();
  drawPieces();
  // draw();//THIS IS WHERE THE PROBLEM IS.. drawing for some reason uses the old state
}
...
function canSelectedMoveToBlock(selectedkey, clickedBlock, enemyPiece)
{
    var bCanMove = false;
